...
Builder 会按科研问题组织内置 recipes。Group labels、字段需求、默认 labels 和默认 styles 来自 `GET /api/recipe-catalog` 暴露的 bundled recipe catalog；这是内置 recipes 的稳定 substrate，不是外部 pack loading。
...
Recipes 会把变量名、列名、样式选择和目标 axes 存进 `FigureSpec`，不会保存原始 DataFrame 数据。`boxplot_by_category` 和 `violin_by_category` 使用 X 和 Y 列以及可选 group 列；因为直接绘制 distribution，error 设置会被忽略。`count_bar` 只使用 X 和可选 group 列；`stacked_bar` 使用 X 和 group 列；两者都会忽略 value/error columns。生成代码仍只 import Matplotlib，并从 live DataFrame 变量计算统计量。
...
在 **Publish** mode 和导出前，FigStudio 会运行确定性的 publication-readiness checks。这些检查会提示没有 data-bearing 图层或 recipe、缺少 axis labels、secondary Y axis 没有 label、multi-panel titles 薄弱、同一 axes 上多个 visual items 缺少 legend labels、受限 panel 中 crowded legend 的 overlap risk，以及 PNG 分辨率偏低。Readiness warnings 是 advisory：它们会显示在 validation card list 中，但只要没有普通 validation errors，export 仍会继续。

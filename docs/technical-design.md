...
FigStudio is a Python-owned local application with a React editor. Python owns data access, validation, Matplotlib rendering, code generation, export, controlled writeback, and packaged static serving. React owns editor state, variable, layer, and recipe controls, annotations, preview display, validation display, FigureSpec import/export, and user actions.

FigStudio 是一个 Python 负责核心能力、本地 React 负责编辑界面的应用。Python 负责数据访问、校验、Matplotlib 渲染、代码生成、导出、受控写回和包内静态资源服务；React 负责编辑器状态、变量、图层和 recipe 控件、注释、预览展示、校验展示、FigureSpec 导入导出和用户操作。
...
- `validation.py`: validates missing variables, missing columns, non-DataFrame recipe sources, missing axes, dimension mismatches, 2D requirements, and log-scale positivity before render/export.
- `codegen.py`: converts `FigureSpec` into plain Matplotlib OO code with no FigStudio runtime dependency. Style settings are emitted through `plt.rc_context` so generated code does not mutate global rcParams.
...
- `validation.py`：在渲染/导出前校验缺失变量、缺失列、非 DataFrame recipe source、缺失 axes、维度不匹配、二维数据要求和 log 坐标轴正值要求。
- `codegen.py`：把 `FigureSpec` 转换为纯 Matplotlib OO 代码，不引入 FigStudio 运行时依赖。样式设置通过 `plt.rc_context` 输出，避免生成代码修改全局 rcParams。
...
`FigureSpec` is the editor state and codegen source of truth. It contains figure size, DPI, row/column layout, axes, plot layers, recipe layers, annotations, style, mode, and whether generated code calls `plt.show()`.

`FigureSpec` 是编辑器状态和 codegen 的唯一来源。它包含图尺寸、DPI、行列布局、axes、plot layers、recipe layers、annotations、style、mode，以及生成代码是否调用 `plt.show()`。
...
`RecipeLayer.kind` supports `mean_sem_line`, `grouped_points`, and `paired_before_after`. Recipe MVP inputs are pandas DataFrame variables with named columns through `RecipeDatasetRef`; specs store variable and column names only, not raw data.

`RecipeLayer.kind` 支持 `mean_sem_line`、`grouped_points` 和 `paired_before_after`。Recipe MVP 输入是通过 `RecipeDatasetRef` 指定列名的 pandas DataFrame 变量；spec 只保存变量名和列名，不保存原始数据。

`DatasetRef` supports both same-variable column mapping and independent variable mapping through `x_variable`, `y_variable`, `z_variable`, and `yerr_variable`. `LayerStyle.colorbar` lets heatmap layers suppress colorbars; contour layers still fall back to the axes colorbar setting when the layer-level value is unset. `FigureStyle.preset` records `custom`, `journal_single`, `journal_double`, `poster`, or `slide`.

`DatasetRef` 通过 `x_variable`、`y_variable`、`z_variable` 和 `yerr_variable` 同时支持同变量列映射和独立变量映射。`LayerStyle.colorbar` 让 heatmap layer 可以关闭 colorbar；contour layer 在未设置 layer-level 值时仍 fallback 到 axes colorbar 设置。`FigureStyle.preset` 记录 `custom`、`journal_single`、`journal_double`、`poster` 或 `slide`。
...
- Recipe code may call methods on existing pandas DataFrame variables but must keep imports limited to Matplotlib.
...
- FigureSpec 中的 recipe entry 保存 recipe 意图和列引用，不保存原始 DataFrame 内容。
- 生成绘图代码不得 import 或依赖 FigStudio。
- Recipe 代码可以调用现有 pandas DataFrame 变量的方法，但 import 必须限于 Matplotlib。

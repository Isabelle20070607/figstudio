...
`GET /api/layer-catalog` exposes the bundled plot-layer catalog used by the editor. The response includes `groups` and `layers`; each layer entry records its `kind`, label, group, role text, required and optional dataset fields, secondary-axis support, 2D/colorbar capability, legend policy, and default style. This catalog is bundled-only in the public beta and is not an external plugin or pack-loading contract.
...
Supported `RecipeLayer.kind` values are `mean_sem_line`, `mean_sem_bar`, `count_bar`, `stacked_bar`, `boxplot_by_category`, `violin_by_category`, `grouped_points`, `paired_before_after`, `ecdf`, `neuro.core.trial_response_timecourse`, and `neuro.ephys.event_rate_timecourse`.

`GET /api/recipe-catalog` exposes the bundled recipe catalog used by the editor. The response includes `groups` and `recipes`; each recipe entry records its `kind`, label, research-question group, role text, required and optional dataset fields, default error behavior, default label behavior, legend grouping behavior, and default style. This catalog is bundled-only in the public beta and is not an external plugin or pack-loading contract.

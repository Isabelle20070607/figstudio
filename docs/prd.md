...
FigStudio is a public beta local figure-workflow tool for turning data already present in a Python session into reproducible, publication-ready Matplotlib figure panels. Users install the Python package, open a local browser editor from a script or notebook, map variables to plot layers or supported statistics recipes, preview authoritative Matplotlib output, export files, save generated OO code, and optionally save the GUI session as a portable `FigureSpec` JSON file.

FigStudio 是一个公开 beta 的本地 figure-workflow 工具，用来把 Python 会话中已有的数据转换成可复现、适合发表的 Matplotlib 图版。用户安装 Python 包后，可从脚本或 Notebook 打开本地浏览器编辑器，映射变量、创建图层或受支持的统计 recipe、预览真实 Matplotlib 输出、导出文件、保存生成的 OO 代码，并可把 GUI 会话保存成可移植的 `FigureSpec` JSON 文件。
...
- Create and edit general statistics recipes for pandas DataFrames: `mean_sem_line`, `grouped_points`, and `paired_before_after`.
...
- Validate missing data, missing columns, non-DataFrame recipe sources, missing axes, shape mismatches, 2D heatmap/contour requirements, and log-scale non-positive data before render.
...
- 为 pandas DataFrame 创建和编辑通用统计 recipe：`mean_sem_line`、`grouped_points` 和 `paired_before_after`。
...
- 在渲染前校验缺失数据、缺失列、非 DataFrame recipe source、缺失 axes、维度不匹配、heatmap/contour 的二维数据要求，以及 log 坐标轴的非正数据。
- 将生成代码写入唯一受控脚本块，或返回 Notebook 替换代码。
- 导入和导出 `.figstudio.json` 会话 spec。
- spec 保存 recipe 意图和列映射，不保存原始 DataFrame 数据。
...
- As a researcher, I want common DataFrame statistics recipes so I can build mean/SEM, grouped point, and paired before/after panels without giving up readable Matplotlib code.
...
- 作为研究者，我希望有常见 DataFrame 统计 recipe，以便生成 mean/SEM、grouped point 和 paired before/after 图版，同时仍保留可读的 Matplotlib 代码。
...
- A script can launch FigStudio, create a DataFrame statistics recipe, render a preview, export/import the spec, and save generated code that computes statistics from the live DataFrame.
- A notebook-style session without `script_path` returns complete replacement code.
- Generated code imports Matplotlib only and can run with the same user variables, including recipe code that calls methods on existing pandas DataFrames.
...
- 脚本能启动 FigStudio，基于 DataFrame 创建统计 recipe、渲染预览、导入导出 spec，并保存会从 live DataFrame 计算统计量的生成代码。
- 不提供 `script_path` 的 Notebook 风格会话能返回完整替换代码。
- 生成代码只 import Matplotlib，并能在相同用户变量下运行，包括调用现有 pandas DataFrame 方法的 recipe 代码。
...
- 捆绑通用统计 recipe 之外的领域 recipe packs。
- 公开 beta 阶段的桌面安装器。

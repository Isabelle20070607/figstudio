...
- Keep browser-level UI smoke coverage for add-layer, add-recipe, preset, annotation, export, import/export, subplot, validation focus, and save-code flows.
...
- 支持第一批面向 pandas DataFrame 的通用统计 recipe：`mean_sem_line`、`grouped_points` 和 `paired_before_after`。
- 保持 recipe spec 轻量：保存变量名、列映射、样式、目标 axes 和 recipe kind，不保存原始 DataFrame 内容。
- 为 add-layer、add-recipe、preset、annotation、export、import/export、subplot、validation focus 和 save-code flow 保持浏览器级 UI 冒烟覆盖。
...
- Add richer panel layout support through `GridSpec` or `subplot_mosaic` without breaking existing axes ids.
- Add project-level style profiles after the generated-code and FigureSpec compatibility contract stays stable.
- Strengthen notebook workflow with better copy/cell output ergonomics and explore an explicit JupyterLab extension flow that still avoids silent notebook file mutation.
- Build a real example gallery that exercises plot layers, general statistics recipes, existing Figure inspection, notebook-style output, and package install flows.
...
- 通过 `GridSpec` 或 `subplot_mosaic` 增强 panel layout，同时不破坏现有 axes id。
- 在 generated-code 和 FigureSpec 兼容性契约稳定后，增加项目级 style profiles。
- 加强 Notebook workflow，改善 copy/cell output 体验，并探索明确授权的 JupyterLab extension flow，仍避免静默修改 notebook 文件。
- 建立真实 example gallery，覆盖 plot layers、通用统计 recipes、existing Figure inspection、notebook-style output 和 package install flows。
- 只有当 Matplotlib 暴露足够 raw 或可复现数据时，才把 existing Figure inspection 扩展为可编辑 statistical recipes。
...
- 在 style profiles 被验证后，增加可选项目级模板，用于统一 figure 样式治理。
- 只有在当前 generated-code contract 稳定后再探索更丰富的样式系统。
- 只有在 Python package 工作流稳定后再探索桌面安装器。
- 只有在内置 recipe 和 gallery 模式清晰后，再探索 plugin 或 template 分发。
...
- 完整 Matplotlib artist coverage，包括所有 custom artist、3D、polar、动画或浏览器原生交互式发布。
- 自动把任意 seaborn、statannotations 或 custom statistical artists 恢复为可编辑 recipe。
- 首个公开 beta 的桌面安装器。
